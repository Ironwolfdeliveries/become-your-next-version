...
export function Orientation({ video }: { video?: { src: string; captions: string; poster?: string } }) {
  ...
  return <section className="orientation-guide" aria-busy={pending}>
    ...
    <div className="orientation-guide-visual">
      ...
      {step === 3 && <><span>{context.daily?.priority ? "Your saved priority" : "Example of a useful first step"}</span><strong>{context.daily?.priority || "The priority you choose"}</strong><span>{context.daily?.action || "One small action that moves your own priority forward."}</span></>}
