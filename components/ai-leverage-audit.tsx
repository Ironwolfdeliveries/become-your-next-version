...
export function AILeverageAudit({ priority, action, kaiLive }: { priority: string; action: string; kaiLive: boolean }) {
  ...
  function openKai() {
    if (!selectedTask || !kaiLive) return;
  ...
  return <section className="ai-leverage" aria-labelledby="ai-leverage-title">
    ...
    {selectedTask ? <div className="ai-leverage-result"><p className="eyebrow">Start here</p><h3>{selectedTask.title}</h3><dl><div><dt>AI can help</dt><dd>{selectedTask.aiRole}</dd></div><div><dt>Your judgment stays in charge</dt><dd>{selectedTask.humanCheck}</dd></div></dl><div className="button-row"><button className="button" type="button" disabled={!kaiLive} onClick={openKai}>{kaiLive ? "Let Kai help me" : "Kai activation pending"}</button><button className="button secondary" type="button" onClick={() => setShowHandoff((shown) => !shown)}>{showHandoff ? "Hide my AI handoff" : "Teach me to use my AI"}</button></div>{!kaiLive ? <p className="field-help">Live Kai is not connected yet. The transferable handoff works now with the AI assistant you prefer.</p> : null}{showHandoff ? <div className="ai-handoff"><label htmlFor="ai-handoff-text">Portable AI handoff</label><textarea id="ai-handoff-text" readOnly rows={16} value={handoff} /><button className="text-button" type="button" onClick={() => void copyHandoff()}>Copy handoff</button><p className="field-help">Review the text, add only necessary context, and never paste passwords, private credentials, or information you are not allowed to share.</p></div> : null}</div> : <p className="ai-leverage-empty">Select one to three areas. Your first selection becomes the recommended starting workflow.</p>}
