...
export function AskKai() {
  ...
  const [responseMode, setResponseMode] = useState<"GUIDED" | "LIVE">("GUIDED");
  const [aiHandoff, setAiHandoff] = useState<{ prompt: string; customize: string } | null>(null);
  ...
  async function submitQuestion(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/kai", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ message: request.message, route: pathname, intent: request.intent, conversationId }) });
      const result = await response.json() as { answer?: string; conversationId?: string; error?: string; mode?: "GUIDED" | "LIVE"; aiHandoff?: { prompt: string; customize: string }; nextAction?: { href: string; label: string } };
      if (!response.ok || !result.answer) throw new Error(result.error ?? "Kai could not respond.");
      setAnswer(result.answer); setConversationId(result.conversationId ?? conversationId); setResponseMode(result.mode ?? "GUIDED"); setAiHandoff(result.aiHandoff ?? null); setNextAction(result.nextAction ?? page.recommendation); setCopyStatus(""); setPrompt(""); setQuickAction(null);
  ...
  async function copyPrompt() {
    if (!aiHandoff) return;
    try { await navigator.clipboard.writeText(aiHandoff.prompt); setCopyStatus("Prompt copied. Review and customize it before sharing."); }
    catch { setCopyStatus("Copy was blocked. Select the prompt and copy it manually."); }
  }
        ...
        <section className="ask-kai-panel" id="ask-kai-panel" role="dialog" aria-label="Ask Kai">
          <header className="ask-kai-panel-head">
            ...
            <div>
              ...
              <small>Guided Kai · page-aware BYNV guidance</small>
            </div>
          ...
          <div className="ask-kai-body">
            ...
            <p className="ask-kai-status" aria-live="polite">
              {error || (preparedRequest ? `${responseMode === "GUIDED" ? "Guided Kai" : "Live Kai"} used ${preparedRequest.context.pageTitle} context.` : "Guided Kai uses rules, the current page, and permitted private account context—without a paid model call.")}
            </p>
            {answer && <div className="ask-kai-answer" aria-live="polite"><small>Kai</small><p>{answer}</p></div>}

            {aiHandoff ? <div className="ask-kai-handoff"><div><small>Use AI for this</small><strong>Portable prompt</strong></div><textarea readOnly rows={12} value={aiHandoff.prompt} aria-label="Portable AI prompt" /><button className="text-button" type="button" onClick={() => void copyPrompt()}>Copy prompt</button><p>{aiHandoff.customize}</p><p className="form-message" role="status" aria-live="polite">{copyStatus}</p></div> : null}
