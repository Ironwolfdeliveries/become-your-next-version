...
export function Assessment() {
  ...
  function choose(value: number) {
    if (Object.keys(answers).length === 0) trackAnalyticsEvent("snapshot_start");
  ...
  function next() {
    ...
    trackAnalyticsEvent("snapshot_complete");
    router.push("/version-score");
  }
  return <div className="assessment-card"><div className="progress"><span style={{ width: `${((step + 1) / questions.length) * 100}%` }} /></div><p className="eyebrow">Version Snapshot · Question {step + 1} of {questions.length}</p><fieldset><legend>{prompt}</legend><div className="scale">{[1,2,3,4,5].map((n) => <button type="button" className={answer === n ? "selected" : ""} aria-pressed={answer === n} key={n} onClick={() => choose(n)}><b>{n}</b><small>{step === 0 ? ["Not clear at all", "Slightly clear", "Somewhat clear", "Very clear", "Completely clear"][n - 1] : n === 1 ? "Not yet" : n === 5 ? "Consistently" : ""}</small></button>)}</div></fieldset><div className="assessment-actions"><button className="text-button" disabled={step === 0} onClick={() => setStep(step - 1)}>← Back</button><button className="button" disabled={!answer} onClick={next}>{step === questions.length - 1 ? "See my Version Snapshot" : "Continue →"}</button></div><p className="fine-print">Your answers remain in this browser until you create or sign in to a secure BYNV account.</p></div>;
}
