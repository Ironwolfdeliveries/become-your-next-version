...
export default async function AccountPage() {
  ...
  const tier = (membership?.tier ?? "foundation") as MembershipTier;
  ...
  return <>
    ...
    <section className="container account-grid">
      ...
      <article className="panel"><p className="eyebrow">Membership</p><h2>{tierName}</h2><dl className="account-details"><div><dt>Status</dt><dd>{membership?.status ?? "free"}</dd></div>{membership?.current_period_end && <div><dt>Current period</dt><dd>Through {new Date(membership.current_period_end).toLocaleDateString()}</dd></div>}{membership?.cancel_at_period_end && <div><dt>Renewal</dt><dd>Ends after the current billing period</dd></div>}</dl><Button href="/membership">Review membership</Button>{membership?.stripe_customer_id && <p className="field-help">Billing management is available from the Membership page.</p>}</article>
