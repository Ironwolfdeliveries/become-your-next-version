...
import { entryGuides, guidePublished } from "@/lib/entry-guides";
...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  })), ...["/start", ...entryGuides.map(g => `/start/${g.slug}`)].map(route => ({ url: `${SITE_URL}${route}`, lastModified: new Date(guidePublished), changeFrequency: "monthly" as const, priority: 0.8 }))];
}
