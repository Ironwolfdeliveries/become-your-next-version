...
export default async function Membership({ searchParams }: { searchParams: Promise<{ billing?: string; invite?: string }> }) {
  ...
  return <><PageHero eyebrow="The Architects" title="Choose the level that fits the work." copy={foundationReady && builderReady && architectReady ? "Begin with 30 days free, continue at the published Foundation launch rate, or choose the deeper membership that fits your work." : "Begin with the BYNV account experience now. Secure paid membership checkout opens after final billing verification."} />
