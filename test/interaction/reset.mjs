...
await mount();await answer("A different direction");await click("Continue");await click("Skip this question");await answer("One new step");await click("Review my summary");
state.daily={id:"old",focus_date:state.today,priority:"Original priority",action:"Original action",completed:false,reflection:"Keep this note",steps:null,check_in:null,recovery:null,cycle_id:"old-cycle",updated_at:"2026-09-17T12:00:00Z"};
await click("Add my action to Today’s Plan");assert.equal(calls[0].url,"/api/account/experience");assert.equal(calls[0].payload.priority,"Original priority");assert.equal(calls[0].payload.reflection,"Keep this note");assert.equal(calls[0].payload.steps.length,2);assert.equal(calls[0].payload.cycle_id,"old-cycle");await unmount();
await mount();await answer("A different direction");await click("Continue");await click("Skip this question");await answer("One new step");await click("Review my summary");state.daily.check_in="progress";
await click("Add my action to Today’s Plan");assert.equal(calls.length,0);assert.ok(getByRole(container,"alert").textContent.includes("check-in"));state.daily=null;await unmount();
